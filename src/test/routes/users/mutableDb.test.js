...
beforeEach(async () => {
  const setup = await setupTeardown.loginSetup(usersRouter, "/users")
  server = setup.server
...
describe("POST /users/:username/blog-posts", () => {
  ...
  beforeEach(async () => {
    ...
    const publicBlogPost = await BlogPost.findOne({
  ...
  describe("Save", () => {
    test("Title", async () => {
      ...
      const blogPosts = await BlogPost.find({ title }).lean().exec()
...
describe("PUT /users/:username/blog-posts/:blogPostId", () => {
  ...
  beforeEach(async () => {
    ...
    privateVersion = await BlogPost.findOne({
    ...
    unpublishedBlogPost = await BlogPost.findOne({
      author: loginUser._id,
  ...
  describe("Discard", () => {
    test("Blog post published", async () => {
      expect(privateVersion.title).not.toBe(publicVersion.title)
      expect(privateVersion.content).not.toBe(publicVersion.content)
      expect(privateVersion.keywords).not.toEqual(publicVersion.keywords)
      ...
      const newPrivateVersion = await BlogPost.findById(privateVersion._id)
        .lean()
        .exec()
    ...
    test("Blog post not published", async () => {
      ...
      // Confirm deletion
      const result = await BlogPost.findById(unpublishedBlogPost._id)
        .lean()
        .exec()
  ...
  describe("Save", () => {
    test("Blog post published", async () => {
      ...
      const blogPost = await BlogPost.findById(privateVersion._id).lean().exec()
    ...
    test("Blog post not published", async () => {
      ...
      const blogPost = await BlogPost.findById(unpublishedBlogPost._id)
        .lean()
        .exec()
  ...
  describe("Publish", () => {
    test("Blog post published", async () => {
      expect(privateVersion.title).not.toBe(title)
      expect(privateVersion.content).not.toBe(content)
      expect(privateVersion.keywords).not.toEqual(keywords)
      ...
      const updatedPrivate = await BlogPost.findById(privateVersion._id)
        .populate("public_version")
        .lean()
        .exec()
    ...
    test("Blog post not published", async () => {
      expect(unpublishedBlogPost.title).not.toBe(title)
      expect(unpublishedBlogPost.content).not.toBe(content)
      expect(unpublishedBlogPost.keywords).not.toEqual(keywords)
      ...
      const updatedPrivate = await BlogPost.findById(unpublishedBlogPost._id)
        .populate("public_version")
        .lean()
        .exec()
      const updatedPublic = updatedPrivate.public_version
...
describe("DELETE /users/:username/blog-posts/:blogPostId", () => {
  ...
  beforeEach(async () => {
    ...
    privateLoginUserBlogPost = await BlogPost.findOne({
      author: loginUser._id,
      $or: [
        { public_version: { $exists: true } },
        { publish_date: { $exists: false } },
  ...
  test("Delete", async () => {
    ...
    const result = await BlogPost.findById(privateLoginUserBlogPost._id)
      .lean()
      .exec()
...
describe("POST /users/:username/reactions", () => {
  ...
  beforeEach(async () => {
    ...
    const loginUserReactions = await Reaction.find({ user: loginUser._id })
      .lean()
      .exec()
    const contentIds = loginUserReactions.map(
      (reaction) => reaction.content.content_id,
    )

    publicBlogPost = await BlogPost.findOne({
      _id: { $nin: contentIds },
      publish_date: { $exists: true },
      public_version: { $exists: false },
    })
  ...
  test("All fields, liked blog post", async () => {
    ...
    const allLikeReactions = await Reaction.find({
      content: {
  ...
  test("All fields, disliked comment", async () => {
    ...
    // confirm creation of reaction
    const allDislikeReactions = await Reaction.find({
      content: {
...
describe("PUT /users/:username/reactions/:reactionId", () => {
  ...
  test("Update comment reaction", async () => {
    const oldReactionCounter = await ReactionCounter.findOne({
      content: {
        content_type: "Comment",
        content_id: comment._id,
      },
    ...
    const newReaction = await Reaction.findById(commentReaction._id)
      .lean()
      .exec()
    ...
    const newReactionCounter = await ReactionCounter.findOne({
      content: {
        content_type: "Comment",
        content_id: comment._id,
      },
    ...
    if (newReaction.reaction_type === "Like") {
      expect(oldReactionCounter.like_count).toBe(
        newReactionCounter.like_count - 1,
      )
      expect(oldReactionCounter.dislike_count).toBe(
        newReactionCounter.dislike_count + 1,
      )
  ...
  test("Update blog post reaction", async () => {
    ...
    // confirm reaction updated
    const newReaction = await Reaction.findById(blogPostReaction._id)
      .lean()
      .exec()
...
describe("DELETE /users/:username/reactions/:reactionId", () => {
  ...
  test("Delete comment reaction", async () => {
    ...
    const result = await Reaction.findById(commentReaction._id).lean().exec()
  ...
  test("Delete blog post reaction", async () => {
    ...
    // confirm reaction deleted
    const result = await Reaction.findById(blogPostReaction._id).lean().exec()
