...
function processSvgFile(filePath) {
  fs.readFile(filePath, "utf8", (err, data) => {
    ...
    pugMixin += `\tsvg(viewBox="${
      svgElement.getAttribute("viewBox") || "0 0 24 24"
    }" fill="${svgElement.getAttribute("fill") || "none"}" stroke="${
      svgElement.getAttribute("stroke") || "currentColor"
    }" stroke-linecap="${
      svgElement.getAttribute("stroke-linecap") || "round"
    }" stroke-linejoin="${
      svgElement.getAttribute("stroke-linejoin") || "round"
    }")&attributes(attrs)\n`
    ...
    const outputFilePath = path.join(
      outputDirPath,
      `${path.basename(filePath, ".svg")}Icon.pug`,
    )
