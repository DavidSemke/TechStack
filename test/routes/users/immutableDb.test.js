...
beforeAll(async () => {
  ;({ server, guestApp, autologinApp, loginUser } =
    await setupTeardown.loginSetup(usersRouter, "/users", false))

  nonLoginUser = await User.findOne({ _id: { $ne: loginUser._id } })
    .lean()
    .exec()
})
...
describe("GET /users/:username/blog-posts/:blogPostId", () => {
  ...
  beforeAll(async () => {
    ...
    publicLoginUserBlogPost = await BlogPost.findOne({
    ...
    privateLoginUserBlogPost = await BlogPost.findOne({
      ...
      .lean()
      .exec()
    privateNonLoginUserBlogPost = await BlogPost.findOne({
...
describe("POST /users/:username/reactions", () => {
  ...
  beforeAll(async () => {
    ...
    privateBlogPost = await BlogPost.findOne({
      $or: [
        { public_version: { $exists: true } },
        { publish_date: { $exists: false } },
      ],
      ...
      .lean()
      .exec()
    reactionlessPublicBlogPost = await BlogPost.findOne({
      _id: { $nin: contentIds },
      publish_date: { $exists: true },
      public_version: { $exists: false },
    })
    ...
    reactionPublicBlogPost = await BlogPost.findOne({
      _id: { $in: contentIds },
      publish_date: { $exists: true },
      public_version: { $exists: false },
    })
...
describe("PUT /users/:username/reactions/:reactionId", () => {
  ...
  beforeAll(async () => {
    ...
    nonLoginUserReaction = await Reaction.findOne({
