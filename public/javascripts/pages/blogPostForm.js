...
function blogPostFormTabListeners() {
  const metadata = document.querySelector(".blog-post-form__metadata")
  const content = document.querySelector(".blog-post-form__content")
  ...
  const contentTab = document.querySelector(".blog-post-form-page__content-tab")
...
function blogPostFormMetadataListeners() {
  const titlePreview = document.querySelector(".blog-post-fragment__title")
...
function blogPostFormSubmitListeners() {
  ...
  const saveButton = document.querySelector(".blog-post-form__save-button")
  ...
  blogPostForm.addEventListener("submit", async (event) => {
    ...
    await formFetch(href, method, blogPostForm, (data) => {
      errorsOccurred = true
      ...
      for (const error of data.errors) {
        ...
        if (!metadataErrors && metadataPaths.includes(error.path)) {
          metadataErrors = true
        } else if (!contentErrors && contentPaths.includes(error.path)) {
          contentErrors = true
        }
      }
      ...
      inputData.content.errors.push(...inputData["word-count"].errors)
...
function blogPostFormSetup() {
  const blogPostFormPage = document.querySelector(".blog-post-form-page")
